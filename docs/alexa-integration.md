...
Dashboard deployment [run 34891787277](https://github.com/Mzoratto/mandate/actions/runs/34891787277) promoted merge commit `c775b1e885ddabe29fbff605ebab36e4ccd1b8d3` and its protected simulated client. Production verification confirmed public `/` returns `200`, unauthenticated `/dashboard` and `/simulator` return `401`, authenticated `/simulator` returns `200` with private `no-store` caching, and a read-only lookup of `M-checkout-live-003` returns `COMPLETED`, one action, zero model tokens, and two of three evidence records independently verified. Desktop and mobile browser renders completed without console errors, and neither the viewer password nor control-plane bearer appeared in HTML. No preparation tool was invoked and no live Mandate was created during deployment verification.
